...
function emitPage(pageKey: string, fields: FieldInfo[], url: string): string {
  ...
  // Track whether list components are used (set during section emission)
  let needsListSectionImport = false;
  ...
  // Build section JSX (imports assembled after, once we know what's used)
  ...
  for (const [sectionName, sectionFields] of sections) {
    ...
    if (sectionRepeating.size > 0) {
      for (const [prefix, items] of sectionRepeating) {
        ...
        // Collect unique sub-keys across all items to determine render body
        const subKeySet = new Map<string, string>(); // subKey → type
        for (const [, subFields] of items) {
          for (const sf of subFields) {
            if (!subKeySet.has(sf.fieldKey)) {
              // Find the original field to get its type
              const firstIdx = [...items.keys()][0];
              const fullKey = `${prefix}.${firstIdx}.${sf.fieldKey}`;
              const orig = fields.find((f) => f.fieldKey === fullKey);
              subKeySet.set(sf.fieldKey, orig?.type ?? "text");
            }
          }
        }

        // Detect if this is a pure link-pair list (only .href + .label sub-keys)
        const subKeys = [...subKeySet.keys()];
        const isLinkPairList =
          subKeys.length === 2 &&
          subKeys.includes("href") &&
          subKeys.includes("label");

        // Detect if items have a link pair among other fields
        const hasLinkPair =
          subKeys.includes("href") && subKeys.includes("label");

        // Emit list via pre-built client wrappers (avoids server→client function prop issue)
        sectionJsx.push(`          {/* validate:list-prefix="${prefix}" */}`);
        needsListSectionImport = true;

        if (isLinkPairList) {
          sectionJsx.push(`          <LinkList itemPrefix="${prefix}" />`);
        } else if (
          subKeys.length === 1 &&
          subKeys[0] === "label"
        ) {
          sectionJsx.push(`          <TextList itemPrefix="${prefix}" />`);
        } else {
          // Build subKeys array for ContentBlockList
          const skEntries: string[] = [];
          for (const [subKey, subType] of subKeySet) {
            if (subKey === "href") continue;
            if (subKey === "label" && hasLinkPair) {
              skEntries.push(`{ key: "", as: "a", type: "link" as const }`);
              continue;
            }
            const rt = ["richtext", "html", "markdown"].includes(subType);
            const tag = subKey.endsWith("title") || subKey.endsWith("heading") ? "h3" : "p";
            skEntries.push(
              `{ key: "${subKey}", as: "${tag}", type: "${rt ? "richtext" : "text"}" as const }`
            );
          }
          sectionJsx.push(
            `          <ContentBlockList itemPrefix="${prefix}" subKeys={[${skEntries.join(", ")}]} />`
          );
        }
      }
    }
  ...
  // Check if EditableLink is actually used in the emitted sections
  const sectionsText = sectionJsx.join("\n");
  const actuallyNeedsLink = sectionsText.includes("<EditableLink");
  const actuallyNeedsRichText = sectionsText.includes("<EditableRichText");
  const actuallyNeedsImage = sectionsText.includes("<EditableImage");
  const actuallyNeedsContent = sectionsText.includes("<EditableContent");
  ...
  if (actuallyNeedsContent) imports.push(`import { EditableContent } from "@/components/EditableContent";`);
  if (actuallyNeedsRichText) imports.push(`import { EditableRichText } from "@/components/EditableRichText";`);
  if (actuallyNeedsImage) imports.push(`import { EditableImage } from "@/components/EditableImage";`);
  if (actuallyNeedsLink) imports.push(`import { EditableLink } from "@/components/EditableLink";`);
  if (needsListSectionImport) {
    const listComponents = new Set<string>();
    if (sectionsText.includes("<LinkList")) listComponents.add("LinkList");
    if (sectionsText.includes("<TextList")) listComponents.add("TextList");
    if (sectionsText.includes("<ContentBlockList")) listComponents.add("ContentBlockList");
    if (listComponents.size > 0) {
      imports.push(`import { ${[...listComponents].join(", ")} } from "@/components/ListSection";`);
    }
  }
